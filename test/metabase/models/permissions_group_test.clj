...
(expect "IntuitoBot"        (:name (perm-group/metabot)))
...
;; (Except for the IntuitoBot, which doesn't get data permissions)
(expect
  false
  (tt/with-temp Database [{database-id :id}]
    (group-has-full-access? (:id (perm-group/metabot)) (perms/object-path database-id))))

;; Attempting to create a data permissions entry for the IntuitoBot should throw an Exception
(expect
  Exception
  (tt/with-temp Database [{database-id :id}]
