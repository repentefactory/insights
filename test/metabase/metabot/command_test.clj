...
(deftest command-test
  ...
  (testing "with only archived cards"
    (mt/with-temp Card [_ {:dataset_query (venues-count-query), :name "Cam's Cool IntuitoBot Card", :archived true}]
      ...
      (when (empty? (db/select Card :archived false))
        (is (= {:response '(Exception. "Card Cam's Cool IntuitoBot Card not found.")
                :messages []}
               (command "show" "Cam's Cool IntuitoBot Card")))
        (is (re= #"You don't have any cards yet\."
                 (metabot.cmd/command "list"))))))
  (testing "with ambiguous matches"
    (mt/with-temp* [Card [{card-1-id :id} {:dataset_query (venues-count-query), :name "Cam's Cool IntuitoBot Card 1"}]
                    Card [{card-2-id :id} {:dataset_query (venues-count-query), :name "Cam's Cool IntuitoBot Card 2"}]]
      (is (=
           {:response
            (list
             'Exception.
             (str "Could you be a little more specific, or use the ID? I found these cards with names that matched:"
                  "\n"
                  (format "%d.  <https://metabase.mysite.com/question/%d|\"Cam's Cool IntuitoBot Card 1\">" card-1-id card-1-id)
                  "\n"
                  (format "%d.  <https://metabase.mysite.com/question/%d|\"Cam's Cool IntuitoBot Card 2\">" card-2-id card-2-id)))
  ...
  (testing "calls the correct `pulse/` functions"
    (mt/with-temp Card [{card-id :id} {:dataset_query (venues-count-query)}]
      (is (= {:response "Ok, just a second..."
             ...
             (command "show" card-id))))
    (mt/with-temp Card [_ {:dataset_query (venues-count-query), :name "Cam's Cool IntuitoBot Card"}]
      (is (= {:response "Ok, just a second..."
              :messages `[(~'post-chat-message!
                           nil
                           (~'create-and-upload-slack-attachments!
