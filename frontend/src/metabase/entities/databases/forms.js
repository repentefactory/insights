...
function getAuthCodeEnableAPILink(engine, details) {
  ...
  if (AUTH_URL_PREFIXES[engine] && details["client-id"]) {
    ...
    if (ENABLE_API_PREFIXES[engine] && projectID) {
      ...
      return (
        <span>
          {t`To use LibraFactory Insights with this data you must enable API access in the Google Developers Console.`}{" "}
          {jt`${(
  ...
  details: {
    fields: ({ id, engine, details = {} } = {}) => [
      ...
      ...(getFieldsForEngine(engine, details, id) || []),
      {
        ...
        description: t`When this is on, LibraFactory Insights will automatically run queries when users do simple explorations with the Summarize and Filter buttons when viewing a table or chart. You can turn this off if querying this database is slow. This setting doesn’t affect drill-throughs or SQL queries.`,
        initial: true,
        hidden: !engine,
      },
      {
        ...
        type: "boolean",
        title: t`This is a large database, so let me choose when LibraFactory Insights syncs and scans`,
        description: t`By default, LibraFactory Insights does a lightweight hourly sync and an intensive daily scan of field values. If you have a large database, we recommend turning this on and reviewing when and how often the field value scans happen.`,
        hidden: !engine,
      },
      {
        name: "refingerprint",
        ...
        description: t`When syncing with this database, LibraFactory Insights will scan a subset of values of fields to gather statistics that enable things like improved binning behavior in charts, and to generally make your LibraFactory Insights instance smarter.`,
        hidden: !engine,
      },
      ...
      },
      {
        ...
        description:
          t`LibraFactory Insights can scan the values present in each field in this database to enable checkbox filters in dashboards and questions. This can be a somewhat resource-intensive process, particularly if you have a very large database.` +
          " " +
          t`When should LibraFactory Insights automatically scan and cache field values?`,
        hidden: !engine || !details["let-user-control-scheduling"],
      },
    ],
    normalize: function(database) {
      if (!database.details["let-user-control-scheduling"]) {
        // If we don't let user control the scheduling settings, let's override them with LibraFactory Insights defaults
