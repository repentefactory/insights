...
(log/info
 (format "\nLibraFactory Insights %s" config/mb-version-string)

 (format "\n\nCopyright © %d LibraFactory Insights, Inc." (.getYear (java.time.LocalDate/now)))

 (str "\n\n"
      (if ee-available?
        (str (deferred-trs "LibraFactory Insights Enterprise Edition extensions are PRESENT.")
             "\n\n"
             (deferred-trs "Usage of LibraFactory Insights Enterprise Edition features are subject to the LibraFactory Insights Commercial License.")
             (deferred-trs "See {0} for details." "https://www.metabase.com/license/commercial/"))
        (deferred-trs "LibraFactory Insights Enterprise Edition extensions are NOT PRESENT."))))
...
(defn- -init-create-setup-token
  ...
  (let [hostname  (or (config/config-str :mb-jetty-host) "localhost")
    ...
    (log/info (u/format-color 'green
                              (str (deferred-trs "Please use the following URL to setup your LibraFactory Insights installation:")
                                   "\n\n"
                                   setup-url
                                   "\n\n")))))
...
(defn- destroy!
  ...
  []
  (log/info (trs "LibraFactory Insights Shutting Down ..."))
  ...
  (log/info (trs "LibraFactory Insights Shutdown COMPLETE")))

(defn init!
  "General application initialization function which should be run once at application startup."
  []
  (log/info (trs "Starting LibraFactory Insights version {0} ..." config/mb-version-string))
  ...
  (log/info (trs "Setting up and migrating LibraFactory Insights DB. Please sit tight, this may take a minute..."))
  (mdb/setup-db!)
  ...
  (log/info (trs "LibraFactory Insights Initialization COMPLETE")))

;;; -------------------------------------------------- Normal Start --------------------------------------------------

(defn- start-normally []
  (log/info (trs "Starting LibraFactory Insights in STANDALONE mode"))
  (try
    ...
    (catch Throwable e
      (log/error e (trs "LibraFactory Insights Initialization FAILED"))
      (System/exit 1))))
...
(defn -main
  "Launch LibraFactory Insights in standalone mode."
  [& [cmd & args]]
  ...
  (if cmd
    ...
    (start-normally))) ; with no command line args just start LibraFactory Insights normally
