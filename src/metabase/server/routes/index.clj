...
(defn- load-template [path variables]
  (try
    ...
    (catch IllegalArgumentException e
      (let [message (trs "Failed to load template ''{0}''. Did you remember to build the Intuito frontend?" path)]
        (log/error e message)
...
(defn- entrypoint
  "Repsonse that serves up an entrypoint into the Intuito application, e.g. `index.html`."
