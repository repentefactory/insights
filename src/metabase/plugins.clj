      ...
      (try
        ...
        (let [path (files/get-path filename)]
          ...
          (assert (Files/isWritable path)
            (trs "LibraFactory Insights does not have permissions to write to plugins directory {0}" filename))
          path)
        ...
        (catch Throwable e
          (log/warn
           e
           (trs "LibraFactory Insights cannot use the plugins directory {0}" filename)
           "\n"
           (trs "Please make sure the directory exists and that LibraFactory Insights has permission to write to it.")
           (trs "You can change the directory LibraFactory Insights uses for modules by setting the environment variable MB_PLUGINS_DIR.")
          ...
          (let [path (files/get-path (System/getProperty "java.io.tmpdir"))]
            (assert (Files/isWritable path)
              (trs "LibraFactory Insights cannot write to temporary directory. Please set MB_PLUGINS_DIR to a writable directory and restart LibraFactory Insights."))
            path))))))
...
(defn- plugins-dir
  "Get a `Path` to the LibraFactory Insights plugins directory, creating it if needed. If it cannot be created for one reason or
  ...
  (for [^Path path (files/files-seq (plugins-dir))
        :when      (and (files/regular-file? path)
                        ...
                        (or (not (str/ends-with? (.getFileName path) "spark-deps.jar"))
                            ...
                            ;; the LibraFactory Insights JAR itself as well as plugins no longer are; Java will throw an Exception
                            ...
                            (log/warn
                             (trs "spark-deps.jar is no longer needed by LibraFactory Insights 0.32.0+. You can delete it from the plugins directory."))))]
    path))
...
(defn- init-plugins! [paths]
  ...
  ;; driver `ojdbc8.jar`) always get initialized (i.e., added to the classpath) first; that way, LibraFactory Insights drivers that
  ;; depend on them (such as Oracle) can be initialized the first time we see them.
  ;;
  ;; In Clojure world at least `false` < `true` so we can use `sort-by` to get non-LibraFactory Insights-plugin JARs in front
...
(defn load-plugins!
  "Load LibraFactory Insights plugins. The are JARs shipped as part of LibraFactory Insights itself, under the `resources/modules` directory (the
  source for these JARs is under the `modules` directory); and others manually added by users to the LibraFactory Insights plugins
  directory, which defaults to `./plugins`.

  When loading plugins, LibraFactory Insights performs the following steps:

  *  LibraFactory Insights creates the plugins directory if it does not already exist.
  *  Any plugins that are shipped as part of LibraFactory Insights itself are extracted from the LibraFactory Insights uberjar (or `resources`
     directory when running with `lein`) into the plugins directory.
  *  Each JAR in the plugins directory that *does not* include a LibraFactory Insights plugin manifest is added to the classpath.
  *  For JARs that include a LibraFactory Insights plugin manifest (a `metabase-plugin.yaml` file), a lazy-loading LibraFactory Insights driver
