...
(defroutes ^{:doc "Top-level ring routes for Intuito."} routes
  ...
  (context "/api" [] (fn [& args]
                       ...
                       ;;
                       ;; if Intuito is not finished initializing, return a generic error message rather than
                       ;; something potentially confusing like "DB is not set up"
                       (if-not (init-status/complete?)
                         {:status 503, :body "Intuito is still initializing. Please sit tight..."}
