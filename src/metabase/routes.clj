...
(defroutes ^{:doc "Top-level ring routes for LibraFactory Insights."} routes
  ...
  (context "/api" [] (fn [& args]
                       ...
                       ;;
                       ;; if LibraFactory Insights is not finished initializing, return a generic error message rather than
                       ;; something potentially confusing like "DB is not set up"
                       (if-not (init-status/complete?)
                         {:status 503, :body "LibraFactory Insights is still initializing. Please sit tight..."}
