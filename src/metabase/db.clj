...
(defn get-db-file
  ...
  (let [
        ...
        ;;
        ;; Tell H2 to defrag when LibraFactory Insights is shut down -- can reduce DB size by multiple GIGABYTES -- see #6510
    ...
    (u/prog1 (merge {:type     (case (keyword protocol)
      ...
      (when (and (= (:type <>) :postgres)
        ...
        (log/warn (trs "Warning: Postgres connection string with `ssl=true` detected.")
                  ...
                  (trs "If LibraFactory Insights fails to launch, please add it and try again.")
...
(defn db-type
  "The type of backing DB used to run LibraFactory Insights. `:h2`, `:mysql`, or `:postgres`."
  ^clojure.lang.Keyword []
  (or (:type @connection-string-details)
      (config/config-kw :mb-db-type)))

(def db-connection-details
  "Connection details that can be used when pretending the LibraFactory Insights DB is itself a `Database` (e.g., to use the Generic
  SQL driver functions on the LibraFactory Insights DB itself)."
  (delay
   (when (= (db-type) :h2)
     (log/warn
      (u/format-color 'red
          (str
           (trs "WARNING: Using LibraFactory Insights with an H2 application database is not recommended for production deployments.")
           " "
...
(defn- print-migrations-and-quit-if-needed!
  ...
  (when (liquibase/has-unrun-migrations? liquibase)
    (log/info (str (trs "Database Upgrade Required")
                   "\n\n"
                   (trs "NOTICE: Your database requires updates to work with this version of LibraFactory Insights.")
                   "\n"
...
(defn db-is-setup?
  "True if the LibraFactory Insights DB is setup and ready."
  ^Boolean []
  @db-setup-finished?)
...
(def ^:dynamic *allow-potentailly-unsafe-connections*
  ...
  whether we can connect to the LibraFactory Insights database, in which case we'd like to allow connections to databases that
  don't exist.

  So we need some way to distinguish the LibraFactory Insights database from other databases. We could add a key to the details
  map specifying that it's the LibraFactory Insights DB, but what if some shady user added that key to another database?

  We could check if a database details map matched `db-connection-details` above, but what if a shady user went
  Meta-LibraFactory Insights and added the LibraFactory Insights DB to LibraFactory Insights itself? Then when they used it they'd have potentially unsafe
  access.

  So this is where dynamic variables come to the rescue. We'll make this one `true` when we use `can-connect?` for the
  LibraFactory Insights DB, in which case we'll allow connection to non-existent H2 (etc.) files, and leave it `false` happily and
...
(s/defn ^:private verify-db-connection
  ...
  ([driver :- s/Keyword, details :- su/Map]
   ...
   (assert (binding [*allow-potentailly-unsafe-connections* true]
     ...
     (trs "Unable to connect to LibraFactory Insights {0} DB." (name driver)))
...
(def ^:dynamic ^Boolean *disable-data-migrations*
  ...
  LibraFactory Insights is launched via `load-from-h2`.  That's because they will end up doing things like creating duplicate
...
(s/defn ^:private type-keyword->descendants :- (su/non-empty #{su/FieldTypeKeywordOrString})
  "Return a set of descendents of LibraFactory Insights `type-keyword`. This includes `type-keyword` itself, so the set will always
