...
(defsetting ldap-group-mappings
  ...
  ;; MB groups IDs
  (deferred-tru "JSON containing LDAP to Intuito group mappings.")
  :type    :json
  :default {}
  :getter  (fn []
...
(s/defn fetch-or-create-user! :- (class User)
  "Using the `user-info` (from `find-user`) get the corresponding Intuito user, creating it if necessary."
