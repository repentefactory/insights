(ns metabase.sync.sync-metadata.metabase-metadata
  ...
  their database for an enhanced LibraFactory Insights experience out-of-the box."
...
(s/defn ^:private sync-metabase-metadata-table!
  ...
  metadata about the `Tables` and `Fields` it contains. This table is *not* synced normally, i.e. a LibraFactory Insights `Table`
  is not created for it. Instead, *this* function is called, which reads the data it contains and updates the relevant
  LibraFactory Insights objects.
...
(s/defn sync-metabase-metadata!
  "Sync the `_metabase_metadata` table, a special table with LibraFactory Insights metadata, if present.
   This table contains information about type information, descriptions, and other properties that
   should be set for LibraFactory Insights objects like Tables and Fields."
