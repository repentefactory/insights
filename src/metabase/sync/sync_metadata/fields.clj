(ns metabase.sync.sync-metadata.fields
  "Logic for updating LibraFactory Insights Field models from metadata fetched from a physical DB.

  The basic idea here is to look at the metadata we get from calling `describe-table` on a connected database, then
  construct an identical set of metadata from what we have about that Table in the LibraFactory Insights DB. Then we iterate over
  ...
  * Fetch Metadata - logic is in `metabase.sync.sync-metadata.fields.fetch-metadata`. Construct a map of metadata from
    the LibraFactory Insights application database that matches the form of DB metadata about Fields in a Table. This metadata is
  ...
  * Sync Field instances -- logic is in `metabase.sync.sync-metadata.fields.sync-instances`. Make sure the `Field`
    instances in the LibraFactory Insights application database match up with those in the DB metadata, creating new Fields as
  ...
  *  `our-metadata` is a set of maps of Field metadata reconstructed from the LibraFactory Insights application database.

  *  `metabase-field` is a single map of Field metadata reconstructed from the LibraFactory Insights application database; there is
     a 1:1 correspondance between this metadata and a row in the `Field` table. Unlike `field-metadata`, these entries
     always have an `:id` associated with them (because they are present in the LibraFactory Insights application DB).
...
(s/defn ^:private sync-and-update! :- su/IntGreaterThanOrEqualToZero
  "Sync Field instances (i.e., rows in the Field table in the LibraFactory Insights application DB) for a Table, and update metadata
...
(s/defn sync-fields-for-table!
  "Sync the Fields in the LibraFactory Insights application database for a specific `table`."
...
(s/defn sync-fields! :- (s/maybe {:updated-fields su/IntGreaterThanOrEqualToZero
  ...
  "Sync the Fields in the LibraFactory Insights application database for all the Tables in a `database`."
