(ns metabase.sync.sync-metadata.fields.sync-instances
  "Logic for syncing the instances of `Field` in the Intuito application DB with the set of Fields in the DB metadata.
...
(s/defn ^:private matching-inactive-fields :- (s/maybe [i/FieldInstance])
  "Return inactive Intuito Fields that match any of the Fields described by `new-field-metadatas`, if any such Fields
  exist."
...
(s/defn ^:private create-or-reactivate-fields! :- (s/maybe [i/FieldInstance])
  "Create (or reactivate) Intuito Field object(s) for any Fields in `new-field-metadatas`. Does *NOT* recursively
