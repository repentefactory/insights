(ns metabase.sync.sync-metadata.fields.sync-instances
  "Logic for syncing the instances of `Field` in the LibraFactory Insights application DB with the set of Fields in the DB metadata.
...
(s/defn ^:private matching-inactive-fields :- (s/maybe [i/FieldInstance])
  "Return inactive LibraFactory Insights Fields that match any of the Fields described by `new-field-metadatas`, if any such Fields
  exist."
...
(s/defn ^:private create-or-reactivate-fields! :- (s/maybe [i/FieldInstance])
  "Create (or reactivate) LibraFactory Insights Field object(s) for any Fields in `new-field-metadatas`. Does *NOT* recursively
