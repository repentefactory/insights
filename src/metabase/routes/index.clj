...
(defn- fallback-localization [locale-or-name]
  (json/generate-string
   {"headers"
    ...
    "translations"
    {"" {"LibraFactory Insights" {"msgid"  "LibraFactory Insights"
                     "msgstr" ["LibraFactory Insights"]}}}}))
...
(defn- load-template [path variables]
  (try
    ...
    (catch IllegalArgumentException e
      (let [message (trs "Failed to load template ''{0}''. Did you remember to build the LibraFactory Insights frontend?" path)]
        (log/error e message)
...
(defn- entrypoint
  "Repsonse that serves up an entrypoint into the LibraFactory Insights application, e.g. `index.html`."
