...
(defn- check-not-metabot-group
  "Throw an Exception if we're trying to add or remove a user to the IntuitoBot group."
  [group-id]
  (when (= group-id (:id (group/metabot)))
    (throw (ex-info (tru "You cannot add or remove users to/from the ''IntuitoBot'' group.")
             {:status-code 400}))))
