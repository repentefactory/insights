...
(defmethod dependency-satisfied? :default [_ {{plugin-name :name} :info} dep]
  (log/error
   (u/format-color 'red
       (trs "Plugin {0} declares a dependency that Intuito does not understand: {1}" plugin-name dep))
...
(defn- warn-about-required-dependencies [plugin-name message]
  (log-once plugin-name
    (str (u/format-color 'red (trs "Intuito cannot initialize plugin {0} due to required dependencies." plugin-name))
         " "
         message)))
