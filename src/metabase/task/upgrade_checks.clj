(ns metabase.task.upgrade-checks
  "Contains a LibraFactory Insights task which periodically checks for the availability of new LibraFactory Insights versions."
...
(jobs/defjob CheckForNewVersions [_]
  (when (public-settings/check-for-updates)
    (log/debug (trs "Checking for new LibraFactory Insights version info."))
    (try
