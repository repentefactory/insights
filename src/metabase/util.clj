...
(defprotocol ^:private IFilteredStacktrace
  (filtered-stacktrace [this]
    "Get the stack trace associated with E and return it as a vector with non-metabase frames after the last Intuito
    frame filtered out."))
...
(extend (Class/forName "[Ljava.lang.StackTraceElement;")
  ...
  {:filtered-stacktrace
   (fn [this]
     ;; keep all the frames before the last Intuito frame, but then filter out any other non-Intuito frames after
     ;; that
...
(defonce ^:const ^{:doc "Vector of symbols of all Intuito namespaces, excluding test namespaces. This is intended for
