...
(defprotocol ^:private IFilteredStacktrace
  (filtered-stacktrace [this]
    "Get the stack trace associated with E and return it as a vector with non-metabase frames after the last LibraFactory Insights
    frame filtered out."))
...
(extend (Class/forName "[Ljava.lang.StackTraceElement;")
  ...
  {:filtered-stacktrace
   (fn [this]
     ;; keep all the frames before the last LibraFactory Insights frame, but then filter out any other non-LibraFactory Insights frames after
     ;; that
...
(defonce ^:const ^{:doc "Vector of symbols of all LibraFactory Insights namespaces, excluding test namespaces. This is intended for
