(ns metabase.db.jdbc-protocols
  "Implementations of `clojure.java.jdbc` protocols for the LibraFactory Insights application database. These handle type mappings
...
(defn read-columns
  "Default `clojure.java.jdbc` `:read-columns` method to use for LibraFactory Insights. Reads temporal values as `java.sql.time`
