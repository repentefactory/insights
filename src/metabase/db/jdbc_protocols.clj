(ns metabase.db.jdbc-protocols
  "Implementations of `clojure.java.jdbc` protocols for the Intuito application database. These handle type mappings
...
(defn read-columns
  "Default `clojure.java.jdbc` `:read-columns` method to use for Intuito. Reads temporal values as `java.sql.time`
