(ns metabase.metabot.websocket
  "Logic for managing the websocket IntuitoBot uses to monitor and reply to Slack messages, specifically a 'monitor thread'
...
(defn stop!
  "Stop all IntuitoBot instances. Clear the current monitor thread ID, which will signal to any existing monitor threads to
...
(defn currently-running?
  "Is the IntuitoBot running?

  Checks whether there is currently a IntuitoBot websocket monitor thread running. (The monitor threads make sure the
...
(defn- reopen-websocket-connection-if-needed!
  ...
  []
  ;; Only open the Websocket connection if this instance is the IntuitoBot
  (when (metabot.instance/am-i-the-metabot?)
    (when (= (.getId (Thread/currentThread)) @websocket-monitor-thread-id)
      (try
        (when (or (not  @websocket)
                  (s/closed? @websocket))
          (log/debug (trs "IntuitoBot WebSocket is closed. Reconnecting now."))
          (connect-websocket!))
        (catch Throwable e
