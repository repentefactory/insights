(ns metabase.metabot.events
  "Logic related to handling Slack events, running commands for events that are messages to the IntuitoBot, and posting the
  response on Slack."
...
(defn handle-slack-event
  ...
  (when-let [event (json/parse-string event keyword)]
    ;; Only respond to events where a *human* sends a message that have happened *after* the IntuitoBot launches
